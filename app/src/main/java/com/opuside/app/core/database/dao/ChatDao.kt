 ...
 * 
 * ✅ ОБНОВЛЕНО (Cache System): Добавлен метод withTransaction для атомарных операций
 */
@Dao
abstract class ChatDao {
    ...
    /**
     * ✅ НОВЫЙ МЕТОД: Выполнить блок операций в транзакции.
     * 
     * Используется для атомарного выполнения нескольких операций БД.
     * Например: удаление старого кеша + вставка нового в одной транзакции.
     * 
     * ВАЖНО: Этот метод должен быть `abstract` (не `open`), чтобы Room
     * смог сгенерировать реализацию с правильной транзакцией.
     * 
     * @param block Блок кода для выполнения в транзакции
     * @return Результат блока
     * @throws Exception если транзакция не удалась (откат автоматический)
     * 
     * Пример использования:
     * ```kotlin
     * chatDao.withTransaction {
     *     chatDao.deleteById(oldId)
     *     chatDao.insert(newMessage)
     * }
     * ```
     */
    @Transaction
    open suspend fun <R> withTransaction(block: suspend () -> R): R {
        // Room автоматически создаст транзакцию
        return block()
    }
